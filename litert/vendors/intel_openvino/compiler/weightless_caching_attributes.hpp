...
// because that header lives under dev_api and is NOT shipped in the OpenVINO
// runtime SDK include tree (@intel_openvino only exposes runtime/include/**).
// The declared OPENVINO_API symbols (copy_weightless_cache_attr / is_copyable)
// are provided by libopenvino at link time; this file only supplies the
// declaration + the WeightlessCacheAttribute layout the NPU tagging pass stamps
// onto Constant rt_info (see openvino_compiler_plugin.cc).
